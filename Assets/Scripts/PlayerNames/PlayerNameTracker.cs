...
namespace PlayerNames
{
    ...
    public class PlayerNameTracker : NetworkBehaviour
    {
        ...
        {
            base.OnStartClient();
            SetName(PlayerPrefs.GetString(PlayerNameInputField.PlayerNamePrefKey));
        ...
        {
            Debug.Log(arg2.ConnectionState);
        ...
        {
            if (op is SyncDictionaryOperation.Add or SyncDictionaryOperation.Set or SyncDictionaryOperation.Remove)
        ...
        {
            var clients = InstanceFinder.ClientManager.Clients; 
            return clients.Values.Select(GetPlayerName).ToList();
        }
    }
}
